...
export function sameOrigin(request:Request){const origin=request.headers.get('origin');if(!origin)return false;const normalize=(o:string)=>o.replace('://127.0.0.1','://localhost').replace('://0.0.0.0','://localhost');return normalize(origin)===normalize(new URL(request.url).origin)}
